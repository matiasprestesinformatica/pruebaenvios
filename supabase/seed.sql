...
-- Note: Your current schema uses TEXT for status/type columns, validated by Zod in the app.
-- If you were using DB ENUMs, they would be dropped and recreated here.
-- Example of how you might drop them if they existed:
...
CREATE TABLE "public"."empresas" (
    ...
    "direccion" TEXT NOT NULL, -- Changed from NULL to NOT NULL
    "latitud" NUMERIC NULL,    -- New column for latitude
    "longitud" NUMERIC NULL,   -- New column for longitude
    "telefono" TEXT NULL,
    "email" TEXT NULL UNIQUE, -- Allow nulls but if value exists, it must be unique
    "notas" TEXT NULL,
    "estado" BOOLEAN NOT NULL DEFAULT TRUE -- Added estado column
);
COMMENT ON TABLE "public"."empresas" IS 'Stores company information including geocoded address.';
COMMENT ON COLUMN "public"."empresas"."latitud" IS 'Latitude of the company address.';
COMMENT ON COLUMN "public"."empresas"."longitud" IS 'Longitude of the company address.';
...
CREATE TABLE "public"."clientes" (
    ...
    "telefono" TEXT NULL, -- Changed to NULL
    "email" TEXT NULL UNIQUE,    -- Changed to NULL, but remains UNIQUE if not NULL
    "notas" TEXT NULL,
...
('b2c3d4e5-f6a7-8901-2345-678901bcdef0', 'Nutrisabor', 'Av. Independencia 456, Mar del Plata', 'ventas@nutrisabor.com', '+542235550200', TRUE, -37.9999, -57.5500);
...
('4068601d-34dc-4983-9834-68c630af1b31', '2025-05-19 23:01:30.937323+00', (SELECT id from clientes WHERE email = 'carlos.lopez@example.com'), 'Defensa 567, San Telmo', 'medium', '2.5', 'asignado_a_reparto', 'df1e184e-71e6-4f00-a78d-b6e93034cd35', -34.6175, -58.3703), -- Buenos Aires, not Mar del Plata
('46990e54-0a24-4756-a6b8-96f7229246c5', '2025-05-19 23:01:30.937323+00', (SELECT id from clientes WHERE email = 'juan.perez@example.com'), 'Honduras 4800, Palermo', 'small', '0.8', 'asignado_a_reparto', 'df1e184e-71e6-4f00-a78d-b6e93034cd35', -34.5833, -58.4294), -- Buenos Aires
...
('6a9dd307-1f2e-4eca-9f8b-f887697c4841', '2025-05-19 23:01:30.937323+00', (SELECT id from clientes WHERE email = 'ana.martinez@example.com'), 'Av. Rivadavia 7000, Flores', 'large', '10.2', 'pending', NULL, -34.6265, -58.4656), -- Buenos Aires
('ee0a5dbe-4542-4b52-89ec-a339e1482f0b', '2025-05-19 23:01:30.937323+00', (SELECT id from clientes WHERE email = 'carlos.lopez@example.com'), 'Defensa 567, San Telmo (Oficina)', 'small', '0.5', 'asignado_a_reparto', 'caad29f1-cf73-42d4-b3d2-658a7814f1c4', -34.6175, -58.3703); -- Buenos Aires
