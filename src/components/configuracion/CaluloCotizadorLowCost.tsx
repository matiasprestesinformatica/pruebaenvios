...
interface CaluloCotizadorLowCostProps {
...
const CaluloCotizadorLowCost: React.FC<CaluloCotizadorLowCostProps> = ({ tarifas }) => {
  ...
  useEffect(() => {
    const loadGoogleMapsScript = () => {
      ...
      if (!apiKey) {
        setError("Falta la configuración del mapa (API Key). Contacta al administrador.");
        setMapApiLoading(false);
        return;
      }
  ...
  const calcularRuta = async () => {
    ...
    if (mapApiLoading || !directionsServiceRef.current || !directionsRendererRef.current || !window.google?.maps?.DirectionsService) { setError("El servicio de mapas no está listo. Intente de nuevo o recargue la página."); return; }
    ...
    <div className="w-full max-w-4xl mx-auto p-4 md:p-6 lg:p-8 bg-card shadow-xl rounded-lg animate-fade-in">
      <div className="mb-6 text-center">
        ...
        <p className="text-muted-foreground mt-1">
          Calculá el costo de tu envío económico. Ingresá las direcciones de origen y destino para obtener una cotización instantánea.
        </p>
      </div>
          ...
          <div>
            <Label htmlFor="direccion-origen-lowcost" className="block text-sm font-medium text-foreground mb-1">
             ...
             <div className="flex items-center gap-2 rounded-md border border-input bg-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
                ...
                <Input id="direccion-origen-lowcost" type="text" value={origen} onChange={(e) => setOrigen(e.target.value)} placeholder="Ej: Av. Colón 2000" className="border-0 focus-visible:ring-0 focus-visible:ring-offset-0 flex-grow p-3"/>
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 rounded-md border border-input bg-background focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2">
                <MapPinIcon className="h-5 w-5 text-muted-foreground ml-3 flex-shrink-0" />
                <Input id="direccion-destino-lowcost" type="text" value={destino} onChange={(e) => setDestino(e.target.value)} placeholder="Ej: Juan B. Justo 1500" className="border-0 focus-visible:ring-0 focus-visible:ring-offset-0 flex-grow p-3"/>
            </div>
          </div>
          </TooltipProvider>
